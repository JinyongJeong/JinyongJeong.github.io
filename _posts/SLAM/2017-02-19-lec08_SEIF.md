...
그렇다면 앞에서 이야기한 sparsification이란 무엇일까? <font color='blue'>sparsification은 filter의 연산량이 matrix의 크기에 종속적이지 않도록, information matrix의 non-zero off-diagonal의 수를 한정하는 방법이다.</fond>
...
이제 2개의 landmark가 관측된 상태에서 로봇이 움직였다. 로봇이 움직이면 motion model에 의해 로봇의 위치가 업데이트 되는데 이때 control input의 uncertainty에 의해 process noise가 발생한다. 따라서 로봇 위치에 불확실성이 추가됨에 따라 로봇위치에 대한 information이 줄어들게 된다(좌상단의 $$3\times3$$ matrix). 이때 landmark 자체의 information(diagonal 값)은 로봇의 위치에 영향을 받지 않으므로 값이 변하지 않는다. 그리고 로봇 위치에 대한 불확실성이 증가하였기 때문에 로봇과 landmark사이에도 불확실성이 증가하므로 information값도 줄어들게 된다(off-diagonal 값). 이때 로봇과 landmark사이의 information은 줄어들면서 landmark간의 information값이 추가된다. 왜냐하면 로봇이 움직이기 전의 관측값에 의해 로봇의 위치로 부터 landmark의 상대위치를 알고 있었으므로 이는 landmark간의 상대위치가 계산가능하다는 의미이다. 따라서 로봇과 landmark간의 information이 줄어들면서, 동시에 이전에 관측되었었던 landmark간의 information이 추가되게 된다.
