...
$$\delta$$는 로봇의 위치와 landmark의 위치 차를 의미하며 $$\sqrt{q}$$는 로봇과 landmark의 거리를 의미한다. 따라서 $$h(\bar{\mu}_t)$$는 비선형 observation model이며 현재 로봇의 위치와, landmark의 위치를 알고 있을 때 예상되는 센서의 observation을 계산할 수 있다. 여기서 $$\bar{\mu}_t$$는 로봇의 현재 위치정보($$\bar{\mu}_{t,x}, \bar{\mu}_{t,y}, \bar{\mu}_{t,\theta}$$)와 landmark의 위치($$\bar{\mu}_{j,x}, \bar{\mu}_{j,y}$$)를 포함하고 있는 $$5\times1$$크기의 vector이다.
...
$$^{low}H_t^i$$에서 low는 아직 matrix의 크기를 조정하기 전의 matrix를 표시하기 위해 사용하였다. Jacobian matrix $$^{low}H_t^i$$ 의 크기는 $$2\times5$$이므로 EKF의 update과정에 적용시키기 위해서 $$F_{x,j}$$ matrix를 사용하여 matrix의 크기를 조절한다. 조절된 matrix인 $$H_t^i$$를 Jacobian으로 사용한다.
...
$$F_{x,j}$$에서 1~3행은 로봇의 위치에 대한 Jacobian term을, 4~5행은 landmark에 대한 jacobian term을 전달하기 위함이며, landmark의 index가 j인 경우 4~5행의 $$2\times2$$의 identity matrix는 $$3+2(j-1)$$ 열 다음에 위치하게 된다. $$F_{x,j}$$가 곱해진 $$H_t^i$$의 크기는 $$2 \times (3+2n)$$이다.
