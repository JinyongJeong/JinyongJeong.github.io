...
EIF도 마찬가지로 Tayor expansion을 통한 1차 선형화를 하고, Jacobian을 이용하여 출력의 Gaussian 분포를 계산한다. motion model과 observation model의 선형화한 함수는 다음과 같다.
...
EKF와 다 른점은 Kalman gain을 구하지 않아도 된다는 점이고, EIF의 prediction 과정에서 비선형 함수의 출력을 계산하기 위해서는 평균값을 다시 계산해야 한다는 것이다. 특히 prediction step에서 Information matrix의 inverse 계산이 필요하기 때문에 계산량이 많다.
...
* EKF에 비해서 수학적으로 조금 더 안정하다고 알려져 있다.
* 실제로는 EIF보다 EKF가 더 많이 사용된다.

하지만 SLAM 분야에서는 EKF보다 EIF가 많이 사용된다. 그 이유는 다음 글에서 설명하도록 한다. 
